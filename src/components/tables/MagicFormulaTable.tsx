...
const STORAGE_KEY_MAGIC_FORMULA = 'magic-formula-filters';

export function MagicFormulaTable({ stocks }: { stocks: Stock[] }) {
	const [sorting, setSorting] = useState<SortingState>([
		{
			id: 'magic_formula_rank',
			desc: false,
		},
	]);
	...
	useEffect(() => {
		const savedFilters = localStorage.getItem(STORAGE_KEY_MAGIC_FORMULA);
		if (savedFilters) {
			try {
	...
	const columns: ColumnDef<Stock>[] = [
		{
			...
			accessorFn: (row) => row.magic_formula_props.magic_formula_rank,
			header: ({ column }) => (
				<DataTableColumnHeader
					column={column}
					title="Rank"
					tooltip="Overall rank based on Magic Formula criteria (lower is better)"
				/>
			),
			cell: ({ row }) => (
				<div className="text-center font-bold">
					{row.original.magic_formula_props.magic_formula_rank}
				</div>
			),
		},
		{
		...
		},
		{
			...
			header: ({ column }) => (
				<DataTableColumnHeader
					column={column}
					title="ROC"
					tooltip="Return on Capital"
				/>
			),
			cell: ({ row }) => (
				<div className="text-center font-medium">
					{formatPercent(row.original.magic_formula_props.roc)}
				</div>
			),
		},
		{
			...
			header: ({ column }) => (
				<DataTableColumnHeader
					...
					tooltip="Earnings / Enterprise Value"
				/>
			),
			cell: ({ row }) => (
				<div className="text-center font-medium">
					{formatPercent(row.original.magic_formula_props.ey)}
				</div>
			),
		},
		{
